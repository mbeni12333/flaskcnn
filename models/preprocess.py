...
def preprocess(digit_image):
	...
	"""
	digit=b64_str_to_np(digit_image)
	digit=crop_img(digit)
	digit=center_img(digit)
	digit=resize_img(digit)
	digit=min_max_scaler(digit, final_range=(0, 1))
	digit=reshape_array(digit)
...
def reshape_array(img_ndarray):
	...
	"""
	digit=np.reshape(img_ndarray, (-1, 1, 28, 28), order='C')
